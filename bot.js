        ...
        try {
                ...
                for (let i = 0; i < attempts; i++) {
                    try {
                        ...
                        if (error.code === 'ETELEGRAM' && error.response && error.response.statusCode === 400) {
                            logger.info(`Skipping post due to Telegram API error (400): ${error.message}`);
                            return null;
                        }
            ...
            switch (post.media_type) {
                case 'VIDEO':
                    ...
                    const videoResult = await retry(async () => {
                ...
                case 'IMAGE':
                    ...
                    const photoResult = await retry(async () => {
                ...
                case 'CAROUSEL_ALBUM':
                    ...
                    if (carouselResponse.data && carouselResponse.data.data) {
                        ...
                        if (mediaGroup.length > 0) {
                            const carouselResult = await retry(async () => {
